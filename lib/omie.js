...
const OMIE_OPERATION_CACHE = Object.freeze({
  ...
  // As páginas completas de pedidos podem ultrapassar o limite de tamanho do
  // Runtime Cache. Elas continuam no cache local/single-flight; o cache
  // distribuído recebe somente a agregação compacta por produto abaixo.
  listar_pedidos_venda: { ttlMs: 10 * 60_000, staleMs: 20 * 60_000, sharedTtlSeconds: 0 },
...
async function readSharedOperationCache(key, policy) {
  if (!process.env.VERCEL || !policy?.sharedTtlSeconds) return null;
...
async function storeSharedOperationCache(key, data, policy) {
  if (!process.env.VERCEL || !policy?.sharedTtlSeconds) return;
  try {
...
export async function callOmie(operationName, suppliedParams = {}, options = {}) {
  ...
  async function loadOmieOperation() {
    if (policy) {
      const shared = await readSharedOperationCache(key, policy);
...
async function listAllMatchingProducts(term, callFn) {
  ...
  do {
    let response;
    try {
      response = await callFn("listar_produtos", {
        pagina: page,
        registros_por_pagina: 100,
        filtrar_apenas_descricao: `%${term}%`
      });
    } catch (error) {
      // O Omie usa fault também quando o próprio filtro não encontra produto
      // na página 1. Para uma busca multi-termo isto significa conjunto vazio,
      // não falha de toda a análise.
...
async function listStockForProducts(productIds, date, callFn) {
  ...
  for (let offset = 0; offset < productIds.length; offset += chunkSize) {
    ...
    do {
      let response;
      try {
        response = await callFn("listar_posicao_estoque", {
          nPagina: page,
          nRegPorPagina: 100,
          dDataPosicao: date,
          cExibeTodos: "S",
...
async function listAllStockLocations(callFn) {
  ...
  do {
    let response;
    try {
      response = await callFn("listar_locais_estoque", { nPagina: page, nRegPorPagina: 100 });
    } catch (error) {
      if (pageIsEmpty(error)) break;
      throw error;
    }
...
async function listAllProductSuppliers(callFn) {
  ...
  do {
    let response;
    try {
      response = await callFn("listar_produtos_fornecedor", {
        pagina: page,
...
// A posição de estoque continua curta (30 s), mas a soma histórica de 90 dias
// muda lentamente. Mantê-la por um dia no cache distribuído evita baixar as
// mesmas dezenas de páginas a cada nova instância serverless.
const SALES_SHARED_CACHE_TTL_SECONDS = 24 * 60 * 60;
...
async function consolidatedSalesMetricsByProduct(referenceDate, callFn, companyScope = "matriz") {
  // ObterResumoProdutos foi validado tecnicamente, mas entrega apenas totais
  // financeiros/documentais e não quantidades por SKU. O fallback autorizado
  // é o relatório consolidado listar_produtos_vendidos_periodo, executado uma
  // vez por empresa e reaproveitado localmente para todos os SKUs.
  ...
  if (cached && Date.now() - cached.createdAt < SALES_AGGREGATION_TTL_MS) {
    callFn.__omieTelemetry?.cacheHit?.("agregacao_vendas_60d");
    return cached.value;
  }
  ...
  callFn.__omieTelemetry?.cacheMiss?.("agregacao_vendas_60d");

  const historyStart = addDays(referenceDate, -(historyDays - 1));
  const report = await invoicedProductsReport({
    data_inicio: formatDate(historyStart),
    data_fim: formatDate(referenceDate),
    ordenar_por: "quantidade"
  }, callFn);
  const byProduct = new Map(report.produtos.map((product) => [
    number(product.id_produto),
    new Map([[60, {
      dias: 60,
      venda_bruta: rounded(product.quantidade_vendida, 3),
      cancelamentos: 0,
      devolucoes: 0,
      venda_liquida: rounded(product.quantidade_vendida, 3)
    }]])
  ]));
  const value = {
    byProduct,
    pages: report.paginas_processadas,
    orders: report.quantidade_pedidos_faturados,
    source: "listar_produtos_vendidos_periodo",
    history_days: 60,
...
export async function analyzeProductReplenishment(
  ...
  const officialHistoryDays = 60;
  const historyStart = addDays(referenceDate, -(officialHistoryDays - 1));
  ...
  const salesAggregation = useFastSalesAggregation
    ? await consolidatedSalesMetricsByProduct(referenceDate, callFn, callFn.__omieEmpresa || "matriz")
    : null;
  ...
  const result = selectedProducts.map((product) => {
    ...
    const demand30 = metrics30 ? dailySalesMetric(metrics30) : null;
    const demand60 = dailySalesMetric(metrics60);
    const demand90 = metrics90 ? dailySalesMetric(metrics90) : null;
    const baseDailyDemand = demand60;
    ...
    return {
      ...
      tendencia: metrics30 ? salesTrend(demand30, demand60, demand90) : "NAO_CALCULADA",
      criterio_demanda: "média diária oficial: quantidade líquida faturada nos últimos 60 dias / 60",
  ...
  return {
    ...
    dias_historico: officialHistoryDays,
    ...
    cache_vendas: useFastSalesAggregation
      ? "Relatório consolidado de vendas por produto dos últimos 60 dias, reaproveitado por 15 minutos na instância e por até 24 horas no cache distribuído."
      : null,
    fonte_vendas: useFastSalesAggregation ? "listar_produtos_vendidos_periodo" : "movimentos_estoque_injetados_para_teste",
    observacao_valores: "O giro oficial usa a quantidade líquida faturada consolidada por produto nos últimos 60 dias; custo, última compra e ajustes de estoque não alteram essa média.",
