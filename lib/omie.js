...
let omieRequestQueue = Promise.resolve();
let nextOmieRequestAt = 0;
...
function isRateLimitResponse(response, data) {
  ...
  return response.status === 429 || message.includes("too many requests") || message.includes("rate limit");
}


async function enqueueOmieRequest(requestFn) {
  const queued = omieRequestQueue.catch(() => undefined).then(async () => {
    const waitMilliseconds = Math.max(nextOmieRequestAt - Date.now(), 0);
    if (waitMilliseconds > 0) await sleep(waitMilliseconds);
    try {
      return await requestFn();
    } finally {
      nextOmieRequestAt = Date.now() + OMIE_MIN_INTERVAL_MS;
    }
  });
  omieRequestQueue = queued.then(() => undefined, () => undefined);
  return queued;
}


export async function retryRateLimitedOmieRequest(requestFn, waitFn = sleep, maxAttempts = OMIE_MAX_ATTEMPTS) {
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    const { response, data } = await requestFn();
    const hasFault = Boolean(data?.faultstring);
    if (response.ok && !hasFault) return data;


    const message = data?.faultstring || data?.message || `Erro HTTP ${response.status} na consulta ao Omie.`;
    if (!isRateLimitResponse(response, data) || attempt === maxAttempts) {
      throw new Error(message);
    }


    await waitFn(750 * (2 ** (attempt - 1)));
  }
...
export async function callOmie(operationName, suppliedParams = {}, options = {}) {
  ...
  const { appKey, appSecret } = credentials(options.empresa || "matriz");
  ...
  return retryRateLimitedOmieRequest(() => enqueueOmieRequest(async () => {
    const response = await fetch(`${BASE_URL}${operation.path}`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "User-Agent": "omie-consulta/0.9.0" },
...
function saleMetricsForWindow(orders, referenceDate, historyDays) {
  const cache = globalThis.__omieSalesAggregationCache || (globalThis.__omieSalesAggregationCache = new Map());
  const key = `${formatDate(referenceDate)}:${historyDays}`;
  const cached = cache.get(key);
  if (cached && Date.now() - cached.createdAt < SALES_AGGREGATION_TTL_MS) return cached.value;

  const value = { byProduct: new Map(), pages: 0, orders: 0 };
  cache.set(key, { createdAt: Date.now(), value });
  return value;
}

async function invoicedSalesMetricsByProduct(referenceDate, historyDays, callFn) {
...
export async function analyzeProductReplenishment(
  ...
  const useFastSalesAggregation = callFn === callOmie;
  const salesAggregation = useFastSalesAggregation
    ? await invoicedSalesMetricsByProduct(referenceDate, dias_historico, callFn)
    : null;
